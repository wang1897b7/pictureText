package com.wsd.text.pict_can.component.TabLayoutView;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    /** 关联ViewPager,用于不想在ViewPager适配器中设置titles数据的情况 */
    public void setViewPager(ViewPager vp, ArrayList<String> aTitles) {
        if (vp == null || vp.getAdapter() == null) {
            throw new IllegalStateException("ViewPager or ViewPager adapter can not be NULL !");
        }

        if (aTitles == null || aTitles.size() == 0) {
            throw new IllegalStateException("Titles can not be EMPTY !");
        }

        if (aTitles.size() != vp.getAdapter().getCount()) {
            throw new IllegalStateException("Titles length must be the same as the page count !");
        }
        String[] titles = new String[aTitles.size()];
    ...
    public void setViewPager(ViewPager vp, ArrayList<String> aTitles, FragmentActivity fa, ArrayList<Fragment> fragments) {
        if (vp == null) {
